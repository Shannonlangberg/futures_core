...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (SpeechRecognition) {
    ...
    recognition.onerror = (event) => {
      ...
      const errorMessages = {
        ...
        'service-not-allowed': 'Microphone permission denied. Using text input.',
        'bad-grammar': 'Speech recognition error. Using text input.',
        'language-not-supported': 'Language not supported. Using text input.'
      };
  ...
  async function processVoiceInput(transcript) {
    ...
    try {
      ...
      // Check if this is a query response (any query-related data)
      const isQueryResponse = data.popup === true || data.report || data.analysis || data.question || data.comparison || data.reports || data.percent_changes || data.years;
      ...
      if (isQueryResponse) {
        ...
        if (window.showQueryModal && typeof window.showQueryModal === 'function') {
          try {
            ...
            // Fallback to showQueryBox if modal fails
            showQueryBox({text: data.text, stats: data.stats, insights: data.insights, report: data.report});
          }
        } else {
          // Fallback to showQueryBox if modal function doesn't exist
          showQueryBox({text: data.text, stats: data.stats, insights: data.insights, report: data.report});
        }
  ...
  function testVoiceRecognition() {
    ...
    console.log('🎤 Recognition is running:', recognitionIsRunning);
  ...
  function initialize() {
    ...
    // Test voice recognition on page load
    setTimeout(() => {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Test voice recognition with Ctrl+V
